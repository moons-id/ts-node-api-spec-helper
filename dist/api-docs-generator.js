 ...
 */
function generateParametersYaml(pathParams, queryParams) {
    let yaml = '';
    // Path parameters
 ...
 */
async function validateMethod(method) {
 ...
 * Validate API path
 */
async function validatePath(rawPath) {
    let path = rawPath;
    while (true) {
        // Empty input
        if (!path.trim()) {
 ...
 */
function extractQueryParams(pathStr) {
    if (!pathStr.includes('?')) {
        return [];
    }
 ...
 */
function extractApiVersion(apiSpecPath) {
    try {
        ...
        console.error(`Error reading API spec: ${error.message}`);
        return null;
    }
}
/**
 ...
 */
function generateOpPath(pathStr) {
    return pathStr
        .split('?')[0]
        .split('/') // Split by /
 ...
 */
function generateOutputPath(apiSpecPath, domain, method, opPath) {
